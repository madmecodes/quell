#!/usr/bin/env bash
# Launch the full Sentinel demo locally: ShopWave (the monitored app) and the
# Sentinel operator dashboard. Loads .env if present.
set -euo pipefail
cd "$(dirname "$0")"
...
echo "starting Sentinel dashboard on http://localhost:8090"
...
echo "  Sentinel console : http://localhost:8090   (detect & prevent)"
